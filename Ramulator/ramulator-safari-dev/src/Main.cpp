...
void run_dramtrace(const Config& configs, Memory<T, Controller>& memory, const string& tracename) {
    ...
    /* run simulation */
    bool stall = false, end = false, row = false;
    int reads = 0, writes = 0, clks = 0;
    long addr = 0, dest_addr = 0;
    ...
    while (!end || memory.pending_requests()){
        if (!end && !stall){
            end = !trace.get_dramtrace_request(addr, type, dest_addr, row);
        }
        ...
        if (!end){
            ...
            if (!stall){
                ...
                else if (type == Request::Type::ENCRYPT || type == Request::Type::DECRYPT){
                    reads++;
                    window.insert();
                    writes++;
                }
                else if (type == Request::Type::ENCRYPTROW || type == Request::Type::DECRYPTROW){
                    reads = reads+8;
                    window.insert();
                    writes = writes+8;
                }
        ...
        clks ++;
        Stats::curTick++;
        // memory clock, global, for Statistics
    }
