...
public:
          ...
          addr_bits(int(HMC::Level::MAX))
    {
        ...
        capacity_per_stack = 8; //spec->channel_width / 8;
    ...
    void tick()
    {
        ...
        for (auto ctrl : ctrls) {
          ...
          //for(int i = 0; i < 8; i++){
            //  ctrl->aes_unit[i] = spec->AES_Unit[i];
          //}
          ctrl->tick();
          //for(int i = 0; i < 8; i++){
            //  spec->AES_Unit[i] = ctrl->aes_unit[i];
          //}
        }
        if (is_active) {
          ramulator_active_cycles++;
    ...
    Packet form_request_packet(const Request& req) {
      ...
      Packet::Command cmd;
      //cout << spec->maxblock_entry.flit_num_bits << " " << spec->source_links << " " << spec->payload_flits << endl;
      switch (int(req.type)) {
    ...
    Packet form_request_packetwr(const Request& req) {
      // All packets sent from host controller are Request packets
      long addr = req.dest_addr;
      int cub = addr / capacity_per_stack;
      long adrs = addr;
      int max_block_bits = spec->maxblock_entry.flit_num_bits;
      clear_lower_bits(addr, max_block_bits);
    ...
    void receive_packets(Packet packet) {
      ...
      req.depart_hmc = clk;
      if (req.type == Request::Type::READ  || ((req.type == Request::Type::ENCRYPT || req.type == Request::Type::DECRYPT || req.type == Request::Type::ENCRYPTROW || req.type == Request::Type::DECRYPTROW) && req.read == false)) {
    ...
    bool send(Request req)
    {
        ...
        //For Encrypt Row and Decrypt Row Requests
        if(req.type == Request::Type::ENCRYPTROW || req.type == Request::Type::DECRYPTROW){
            //Assign the corresponding columns to the address vectors
            req.addr_vec0 = req.addr_vec;
            req.addr_vec2 = req.addr_vec;
            req.addr_vec3 = req.addr_vec;
            req.addr_vec4 = req.addr_vec;
            req.addr_vec5 = req.addr_vec;
            req.addr_vec6 = req.addr_vec;
            req.addr_vec7 = req.addr_vec;

            req.addr_vec[int(HMC::Level::Column)] = 0;
            req.addr_vec0[int(HMC::Level::Column)] = 1;
            req.addr_vec2[int(HMC::Level::Column)] = 2;
            req.addr_vec3[int(HMC::Level::Column)] = 3;
            req.addr_vec4[int(HMC::Level::Column)] = 4;
            req.addr_vec5[int(HMC::Level::Column)] = 5;
            req.addr_vec6[int(HMC::Level::Column)] = 6;
            req.addr_vec7[int(HMC::Level::Column)] = 7;
            
            //Prepare the destination address
            
            req.row_addr0.resize(addr_bits.size());
            clear_higher_bits(req.dest_addr, max_address-1ll);
            long addr = req.dest_addr;
            ...
            switch(int(type)) {
                case int(Type::RoCoBaVa): {
                  int max_block_col_bits =
                      spec->maxblock_entry.flit_num_bits - tx_bits;
                  req.row_addr0[int(HMC::Level::Column)] =
                      slice_lower_bits(addr, max_block_col_bits);
                  req.row_addr0[int(HMC::Level::Vault)] =
                      slice_lower_bits(addr, addr_bits[int(HMC::Level::Vault)]);
                  req.row_addr0[int(HMC::Level::Bank)] =
                      slice_lower_bits(addr, addr_bits[int(HMC::Level::Bank)]);
                  req.row_addr0[int(HMC::Level::BankGroup)] =
                      slice_lower_bits(addr, addr_bits[int(HMC::Level::BankGroup)]);
                  int column_MSB_bits =
                    slice_lower_bits(
                        addr, addr_bits[int(HMC::Level::Column)] - max_block_col_bits);
                  req.row_addr0[int(HMC::Level::Column)] =
                    req.row_addr0[int(HMC::Level::Column)] | (column_MSB_bits << max_block_col_bits);
                  req.row_addr0[int(HMC::Level::Row)] =
                      slice_lower_bits(addr, addr_bits[int(HMC::Level::Row)]);
                }
                break;
                case int(Type::RoBaCoVa): {
            ...
            //Assign the corresponding address to the row vector
            
            req.row_addr1 = req.row_addr0;
            req.row_addr2 = req.row_addr0;
            req.row_addr3 = req.row_addr0;
            req.row_addr4 = req.row_addr0;
            req.row_addr5 = req.row_addr0;
            req.row_addr6 = req.row_addr0;
            req.row_addr7 = req.row_addr0;

            req.row_addr0[int(HMC::Level::Column)] = 0;
            req.row_addr1[int(HMC::Level::Column)] = 1;
            req.row_addr2[int(HMC::Level::Column)] = 2;
            req.row_addr3[int(HMC::Level::Column)] = 3;
            req.row_addr4[int(HMC::Level::Column)] = 4;
            req.row_addr5[int(HMC::Level::Column)] = 5;
            req.row_addr6[int(HMC::Level::Column)] = 6;
            req.row_addr7[int(HMC::Level::Column)] = 7;
            
        }
        
        //For Enc and Dec Requests
        if( req.type == Request::Type::ENCRYPT || req.type == Request::Type::DECRYPT){
            req.addr_vec2.resize(addr_bits.size());
            clear_higher_bits(req.dest_addr, max_address-1ll);
            long addr = req.dest_addr;
        ...
        switch(int(type)) {
          case int(Type::RoCoBaVa): {
            int max_block_col_bits =
                spec->maxblock_entry.flit_num_bits - tx_bits;
            req.addr_vec2[int(HMC::Level::Column)] =
                slice_lower_bits(addr, max_block_col_bits);
            req.addr_vec2[int(HMC::Level::Vault)] =
                slice_lower_bits(addr, addr_bits[int(HMC::Level::Vault)]);
            req.addr_vec2[int(HMC::Level::Bank)] =
                slice_lower_bits(addr, addr_bits[int(HMC::Level::Bank)]);
            req.addr_vec2[int(HMC::Level::BankGroup)] =
                slice_lower_bits(addr, addr_bits[int(HMC::Level::BankGroup)]);
            int column_MSB_bits =
              slice_lower_bits(
                  addr, addr_bits[int(HMC::Level::Column)] - max_block_col_bits);
            req.addr_vec2[int(HMC::Level::Column)] =
              req.addr_vec2[int(HMC::Level::Column)] | (column_MSB_bits << max_block_col_bits);
            req.addr_vec2[int(HMC::Level::Row)] =
                slice_lower_bits(addr, addr_bits[int(HMC::Level::Row)]);
          }
          break;
          case int(Type::RoBaCoVa): {
        ...
        /*
        Packet packetwr = form_request_packetwr(req);

        Link<HMC>* linkwr =
            logic_layers[0]->host_links[packetwr.tail.SLID.value].get();
        ...
        */
        ...
        if (packet.total_flits <= link->slave.available_space()) {
          ...
          if(req.type == Request::Type::ENCRYPT || req.type == Request::Type::DECRYPT){
              ...
              ++num_read_requests[coreid];
            ++incoming_read_reqs_per_channel[req.addr_vec[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec2[int(HMC::Level::Vault)]];
            ++num_write_requests[coreid];
          }
          if(req.type == Request::Type::ENCRYPTROW || req.type == Request::Type::DECRYPTROW){
            ++num_read_requests[coreid];
            ++incoming_read_reqs_per_channel[req.addr_vec[int(HMC::Level::Vault)]];
            /*
            ++incoming_read_reqs_per_channel[req.addr_vec0[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec2[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec3[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec4[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec5[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec6[int(HMC::Level::Vault)]];
            ++incoming_read_reqs_per_channel[req.addr_vec7[int(HMC::Level::Vault)]];
*/
            ++num_write_requests[coreid];
          }
