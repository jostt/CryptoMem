...
void Switch<T>::tick() {
  ...
  for (auto vault_ctrl : vault_ctrls) {
    ...
    if((packet.req.type == Request::Type::ENCRYPT || packet.req.type == Request::Type::DECRYPT || packet.req.type == Request::Type::ENCRYPTROW || packet.req.type == Request::Type::DECRYPTROW) && !packet.req.written){
     //Packet packetwr = packet;
      //cout << "in if" << endl;
      
      packet.req.written = true;
      long addr = packet.req.dest_addr;
      int cub = packet.header.CUB.value;
      long adrs = addr;
      int max_block_bits = 6;
      addr >>= max_block_bits;
      int slid = addr % 4;
      int tag = -1; // may return -1 when no available tag // TODO recycle tags when request callback
      int lng = packet.req.type == Request::Type::READ ?
                                                1 : 1 + 2;
      Packet::Command cmd = write_cmd_map[3];
      
      Packet packetwr(Packet::Type::REQUEST, cub, adrs, tag, lng, slid, cmd);
      packetwr.req = packet.req;
      packetwr.req.read = true;
      if(!packet.req.row_mode){
         packetwr.req.done = true;
      }
      packetwr.req.written = true;
      packetwr.req.is_first_command = true;
      Link<HMC>* linkwr =
        logic_layer->host_links[packetwr.tail.SLID.value].get();
        linkwr->slave.receive(packetwr);
