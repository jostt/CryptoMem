...
public:
    ...
    struct EncQueue {
        list<Request> q;
        unsigned int max = 1000; // TODO queue qize
        unsigned int size() {return q.size();}
    };
    ...
    EncQueue encryptq; // queue for requests read but not encrypted yet
    ...
    bool aes_unit[8] = {false, false, false, false, false, false, false, false};
    ...
    /* Member Functions */
    Queue& get_queue(Request::Type type, bool read)
    {
        switch (int(type)) {
            case int(Request::Type::READ): return readq;
            case int(Request::Type::WRITE): return writeq;
            case int(Request::Type::ENCRYPTROW):
                if(read){
                    return writeq;
                } else {
                    return readq;
                }  
            case int(Request::Type::DECRYPTROW):
                if(read){
                    return writeq;
                } else {
                    return readq;
                } 
            case int(Request::Type::ENCRYPT):
                if(read){
                    return writeq;
                } else {
                    return readq;
                }
            case int(Request::Type::DECRYPT):
                if(read){
                    return writeq;
                } else {
                    return readq;
    ...
    {
        Queue& queue = get_queue(req.type, req.read);
        ...
        if (req.type == Request::Type::READ && find_if(writeq.q.begin(), writeq.q.end(),
            ...
            readq.q.pop_back();
         } else if (req.type == Request::Type::ENCRYPTROW && !(req.read) && find_if(writeq.q.begin(), writeq.q.end(),
                [req](Request& wreq){ return req.addr == wreq.addr;}) != writeq.q.end()){
                    req.depart = clk + 1;
                    pending.push_back(req);
                    readq.q.pop_back();
        } else if (req.type == Request::Type::DECRYPTROW && !(req.read) && find_if(writeq.q.begin(), writeq.q.end(),
                [req](Request& wreq){ return req.addr == wreq.addr;}) != writeq.q.end()){
                    req.depart = clk + 1;
                    pending.push_back(req);
                    readq.q.pop_back();
         } else if (req.type == Request::Type::ENCRYPT && !(req.read) && find_if(writeq.q.begin(), writeq.q.end(),
                [req](Request& wreq){ return req.addr == wreq.addr;}) != writeq.q.end()){
            req.depart = clk + 1;
            pending.push_back(req);
            readq.q.pop_back();
         } else if (req.type == Request::Type::DECRYPT && !(req.read) && find_if(writeq.q.begin(), writeq.q.end(),
                [req](Request& wreq){ return req.addr == wreq.addr;}) != writeq.q.end()){
            req.depart = clk + 1;
            pending.push_back(req);
            readq.q.pop_back();
    ...
    Packet form_response_packet(Request& req) {
      // All packets sent from host controller are Request packets
        //printf("incoming packets buffer: %d, end: %d\n", incoming_packets_buffer.find(req.reqid), incoming_packets_buffer.end());
      ...
      int lng = ((req.type == Request::Type::WRITE) || ((req.type == Request::Type::ENCRYPT || req.type == Request::Type::DECRYPT || req.type == Request::Type::ENCRYPTROW || req.type == Request::Type::DECRYPTROW) && req.read)) ?
                1 : 1 + channel->spec->payload_flits;
      //TODO: is this correct for Request::Type::ENCRYPT
      ...
      if(req.type == Request::Type::ENCRYPT || req.type == Request::Type::DECRYPT || req.type == Request::Type::ENCRYPTROW || req.type == Request::Type::DECRYPTROW){
    ...
    void tick()
    {
        ...
        if (pending.size()) {
          ...
          if (req.depart <= clk) {
            if (req.depart - req.arrive > 1) {  
                if(req.addr_vec.data() == req.addr_vec2.data() || req.type == Request::Type::READ){
        ...
        //Check for Encryption/Decryption Requests
        if(req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW){
            if(req->read){
                //If they are read and not yet encrypted /decrypted, enter them in the queue
                if(!req->encryption_done){
                    if(HMC::AES_Unit[req->addr_vec[int(HMC::Level::Vault)] % 8] == false){
                        HMC::AES_Unit[req->addr_vec[int(HMC::Level::Vault)] % 8] = true;
                        req->set_by_me = true;
                        HMC::Just_Reset = 0;
                        HMC::Holding = req->reqid;
                        
                    } else {
                        if(req->set_by_me && (HMC::Was_Reset_Held_By == req->reqid)){
                            req->set_by_me = false;
                        }
                    }
                    
                    if(req->encryption_counter < req->encryption_cycles){
                         //As long as the encryption is not done, don't do anything this cycle
                        if(req->set_by_me){
                            req->encryption_counter++;
                        } else {
                            if(HMC::Just_Reset < 5){
                                req->counter = 0;
                            }
                            req->counter++;
                            HMC::Just_Reset++;
                            if(req->counter > req->encryption_cycles && HMC::Just_Reset > req->encryption_cycles){
                                HMC::AES_Unit[req->addr_vec[int(HMC::Level::Vault)] % 8] = false;
                                req->counter = 0;
                                HMC::Just_Reset = 0;
                                HMC::Was_Reset_Held_By = HMC::Holding;
                                HMC::Holding = -1;
                            }
                        }
                          return;
                        } else {
                          req->encryption_done = true;
                          if(req->set_by_me){
                              HMC::AES_Unit[req->addr_vec[int(HMC::Level::Vault)] % 8] = false;
                              //req->set_by_me = false;
                          }
        ...
        if (req->is_first_command) {
           //printf("is first command\n");
          req->is_first_command = false;
          int coreid = req->coreid;
          if (req->type == Request::Type::READ || req->type == Request::Type::WRITE ||  req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW) {
            channel->update_serving_requests(req->addr_vec.data(), 1, clk);
          }
          if (req->type == Request::Type::READ) {
          ...
          } else if (((req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW) && req->read) ){
              if (is_row_hit(req)) {
                ++(*write_row_hits)[coreid];
                ++(*row_hits);
            } else if (is_row_open(req)) {
            ...
            (*write_transaction_bytes) += req->transaction_bytes;
          } else if (req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW){
            (*queueing_latency_sum) += clk - req->arrive;
            if (is_row_hit(req)) {
                ++(*read_row_hits)[coreid];
                ++(*row_hits);
        ...
        //printf("tick2\n");
        if(req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW){
            if(cmd == HMC::Command::WR || cmd == HMC::Command::RD){
                
            } else { 
                return;
            }
        } else if((cmd != channel->spec->translate[int(req->type)] )){
            return;
        }

        // set a future completion time for read requests
        if (req->type == Request::Type::READ) {
           // printf("Request-Type: Read\n");
            --req->burst_count;
        ...
        else if ((req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT) && req->read){
            //printf("Request-Type: New write\n");
            --req->burst_count;
            if (req->burst_count == 0) {
             req->written = true;
             Packet packet = form_response_packet(*req);
        ...
        } else if (req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT){
             //printf("Request-Type: New read\n");
                --req->burst_count;
            //if(req->col_counter == 7){
              //      req->read = true;
                //}
            if (req->burst_count == 0) {
              req->depart = clk + channel->spec->read_latency;
         ...
         else if ((req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW) && req->read){
            //printf("Request-Type: New write\n");
                       --req->burst_count;
            if (req->burst_count == 0) {
             req->written = true;
        ...
        } else if (req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW){
             //printf("Request-Type: New read\n");
                --req->burst_count;
        ...
        if (req->burst_count == 0) { 
            switch(int(req->type)){
                case int(Request::Type::ENCRYPT):
                    if(!req->done){
                        req->read = true;
                        vector<int> intermediate = req->addr_vec;
                        req->addr_vec = req->addr_vec2;
                        req->addr_vec2 = intermediate;
                        req->done = true;
                        Packet packet = form_response_packet(*req);
                        response_packets_buffer.push_back(packet);
                        queue->q.erase(req);
                ...
                case int(Request::Type::ENCRYPTROW): 
                    if(!req->done){
                                if(req->row_mode && req->col_counter < 7 && !req->read){
                            req->burst_count = channel->spec->burst_count;
                            req->transaction_bytes = channel->spec->payload_flits * 16;
                            switch(req->col_counter){
                                case 0:
                                    req->intermediate = req->addr_vec;
                                    req->addr_vec = req->addr_vec0;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    req->addr_vec = req->addr_vec3;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 3:
                                    req->addr_vec = req->addr_vec4;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 4:
                                    req->addr_vec = req->addr_vec5;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 5:
                                    req->addr_vec = req->addr_vec6;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 6:
                                    req->addr_vec = req->addr_vec7;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                default: 
                        ...
                        } else if (req->row_mode && req->read == true){
                            if(req->col_counter == 7 && !req->once){
                                //req->read = true;
                                //queue->q.erase(req);
                                req->col_counter = 0;
                                req->once = true;
                            ...
                            req->burst_count = channel->spec->burst_count;
                            req->transaction_bytes = channel->spec->payload_flits * 16;
                            req->encryption_done = false;
                            req->encryption_counter = 0;
                            switch(req->col_counter){
                                case 0:
                                    req->addr_vec = req->row_addr1;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 1:
                                    req->addr_vec = req->row_addr2;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 2:
                                    req->addr_vec = req->row_addr3;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 3:
                                    req->addr_vec = req->row_addr4;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 4:
                                    req->addr_vec = req->row_addr5;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 5:
                                    req->addr_vec = req->row_addr6;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    break;
                                case 6:
                                    req->addr_vec = req->row_addr7;
                                    channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                                    req->col_counter++;
                                    req->done = true;
                                    break;
                            ...
                            req->burst_count = channel->spec->burst_count;
                            req->transaction_bytes = channel->spec->payload_flits * 16;
                            //Enable these lines for 0x.. E 0x...
                            //printf("addr: %ld, dest_addr: %ld\n", req->addr, req->dest_addr);
                             * 
                              */
                            if(req->row_mode){
                                req->addr_vec = req->row_addr0;
                            } else {
                                vector<int> intermediate = req->addr_vec;
                                req->addr_vec = req->addr_vec2;
                                req->addr_vec2 = intermediate;
                            ...
                            //channel->update_serving_requests(req->addr_vec.data(), 1, clk);
                            //channel->update_serving_requests(req->addr_vec2.data(), 1, clk);
                            //req->pre = false;
                            //req->done = true;
                        }
                    } else {
...
private:
    typename HMC::Command get_first_cmd(list<Request>::iterator req)
    {
        ...
        if (!no_DRAM_latency) {
            if(req->type == Request::Type::ENCRYPT || req->type == Request::Type::DECRYPT || req->type == Request::Type::ENCRYPTROW || req->type == Request::Type::DECRYPTROW){
                if(req->burst_count > 0 && !(req->read)){
                    //cout << req->addr_vec.data() << " " << req->addr_vec2.data() << endl;
                    //if(req->row_mode && req->col_counter == 7){
                      //  req->read = true;
                    //}
                    return channel->decode(HMC::Command::RD, req->addr_vec.data());
                } else {
                    //cout << req->addr_vec.data() << " " << req->addr_vec2.data() << endl;
                    return channel->decode(HMC::Command::WR, req->addr_vec.data());
                }
            }
    ...
    void issue_cmd(typename HMC::Command cmd, const vector<int>& addr_vec)
    {
        ...
        if (!no_DRAM_latency) {
          ...
          //rowtable->table.begin()
          //cout << *addr_vec.begin() << " " << *(addr_vec.begin()+1) << " " << *(addr_vec.begin()+2) << endl;
          //cout << *addr_vec2.begin() << " " << *(addr_vec2.begin()+1) << " " << *(addr_vec2.begin()+2) << endl;
          rowtable->update(cmd, addr_vec, clk);
          
        } else {
          // still have bandwidth restrsiction (update timing for RD/WR requets)
