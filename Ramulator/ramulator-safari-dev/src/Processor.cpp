...
bool Trace::get_dramtrace_request(long& req_addr, Request::Type& req_type, long& req_dest_addr, bool& row)
{
    req_dest_addr = 0;
    string line;
    ...
    pos = line.find_first_not_of(' ', pos+1);
    string newline = line.substr(pos+1, line.length());
        
    if (pos == string::npos || line.substr(pos)[0] == 'R')
        req_type = Request::Type::READ;
    else if (line.substr(pos)[0] == 'W'){
        req_type = Request::Type::WRITE;
    } else if (line.substr(pos)[0] == 'E' && line.substr(pos)[1] == 'R') {
        req_type = Request::Type::ENCRYPTROW;
        row = true;
        newline = line.substr(pos+3, line.length());
        if(line.substr(pos)[3] == '0'){
            req_dest_addr = std::stoul(newline, &pos, 16);
        }
    } else if (line.substr(pos)[0] == 'D' && line.substr(pos)[1] == 'R') {
        req_type = Request::Type::DECRYPTROW;
        row = true;
        newline = line.substr(pos+3, line.length());
        if(line.substr(pos)[3] == '0'){
            req_dest_addr = std::stoul(newline, &pos, 16);
        }
    }else if (line.substr(pos)[0] == 'D'){
        req_type = Request::Type::DECRYPT;
        if(newline.substr(0)[1] == '0'){
            req_dest_addr = std::stoul(newline, &pos, 16);
        }
    } else if (line.substr(pos)[0] == 'E'){
        req_type = Request::Type::ENCRYPT;
        if(newline.substr(0)[1] == '0'){
            req_dest_addr = std::stoul(newline, &pos, 16);
        }
    }   else assert(false);
