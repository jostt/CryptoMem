...
public:
    ...
    map<int, bool> read_state;
    map<int, bool> write_state;
    map<int, int> counter;
...
{
    //printf("check: %ld, cmd: %d, addr: %d, clk: %ld, scope: %d, level: %d\n", next[int(cmd)], cmd, *addr, clk, spec->scope[int(cmd)], level);
    //printf("next: %ld, cmd: %d, clk: %ld\n", next[int(cmd)], cmd, clk);
...
void DRAM<T>::update_serving_requests(const int* addr, int delta, long clk) {
  //cout << addr[0] << " " << addr[1] << " " << addr[3] << " " << addr[4] << " " << " " << cur_serving_requests << " " << delta << endl;
  ...
  //printf("cur_serving_requests: %d\n", cur_serving_requests);
